...
from lab_utils_logistic import np, plt, dlc, predict_logistic, plot_data, zscore_normalize_features
...
class overfit_example():
    ...
    def logistic_data(self, redraw=False):
        if not redraw:
            ...
            y_train = X_train[:, 1]+0.5 > X_train[:, 0]**2 + \
                0.5*np.random.rand(m)  # quadratic + random
        ...
        self.ax[0].plot(self.x_ideal, self.y_ideal, "--",
                        color="orangered", label="ideal", lw=1)
    ...
    def linear_data(self, redraw=False):
        ...
        self.ax[0].plot(self.x_ideal, self.y_ideal, "--",
                        color="orangered", label="y_ideal", lw=1)
        self.ax[0].set_title(
            "OverFitting Example: Regression Data Set (quadratic with noise)", fontsize=14)
    ...
    def add_data_logistic(self, event):
        if event.inaxes == self.ax[0]:
            ...
            if event.key is None:  # shift not pressed
                self.ax[0].scatter(x0_coord, x1_coord,
                                   marker='x', s=10, c='red', label="y=1")
                self.y = np.append(self.y, 1)
            else:
                self.ax[0].scatter(x0_coord, x1_coord, marker='o', s=10, label="y=0", facecolors='none',
                                   edgecolors=dlc['dlblue'], lw=3)
                self.y = np.append(self.y, 0)
            self.X = np.append(self.X, np.array(
                [[x0_coord, x1_coord]]), axis=0)
