...
from lab_utils_logistic import sigmoid, dlblue, dlorange, np, plt, compute_cost_matrix
...
def compute_cost_logistic_sq_err(X, y, w, b):
    ...
    for i in range(m):
        z_i = np.dot(X[i], w) + b
        # add sigmoid to normal sq error cost for linear regression
        f_wb_i = sigmoid(z_i)
...
def plt_logistic_cost(X, y):
    ...
    for i in range(points.shape[0]):
        w, b = points[i]
        cost[i] = compute_cost_matrix(
            X.reshape(-1, 1), y, w, b, logistic=True, safe=True)
    cost = cost.reshape(wx.shape)
