...
from lab_utils_logistic import np, plt, dlc, dlcolors, sigmoid, compute_cost_matrix, gradient_descent
...
class plt_quad_logistic:
    ...
    def __init__(self, x_train, y_train, w_range, b_range):
        ...
        width = 0.2
        axcalc = plt.axes(
            [pos[1, 0]-width, pos[1, 1]-h, width, h])  # lx,by,w,h
        ...
        self.cid = fig.canvas.mpl_connect(
            'button_press_event', self.click_contour)
        self.bcalc = Button(
            axcalc, 'Run Gradient Descent \nfrom current w,b (click)', color=dlc["dlorange"])
...
class data_plot:
    ...
    def __init__(self, ax, x_train, y_train, w, b):
        ...
        # leave plot scales the same after initial setup
    ...
    def plt_tumor_data(self):
        ...
        pos = y == 1
        neg = y == 0
        self.ax.scatter(x[pos], y[pos], marker='x',
                        s=80, c='red', label="malignant")
    ...
    def mk_cost_lines(self, firsttime=False):
        ...
        for p in zip(self.x_train, self.y_train):
            ...
            c_p = compute_cost_matrix(
                p[0].reshape(-1, 1), p[1], np.array(self.w), self.b, logistic=True, lambda_=0, safe=True)
            c_p_txt = c_p
            a = self.ax.vlines(p[0], p[1], f_wb_p, lw=3,
        ...
        c = self.ax.text(0.05, 0.02, cstr,
                         transform=self.ax.transAxes, color=dlc["dlpurple"])
...
class contour_and_surface_plot:
    ...
    def __init__(self, axc, axs, x_train, y_train, w_range, b_range, w, b):
        ...
        ### plot contour ###
        CS = axc.contour(tmp_w, tmp_b, np.log(z), levels=12,
        ...
        axs.plot_surface(tmp_w, tmp_b, z,  cmap=cm.jet,
        ...
        # initialize an empty path, avoids existance check
    ...
    def update_contour_wb_lines(self, w, b, firsttime=False):
        ...
        if not firsttime:
            for artist in self.dyn_items:
                artist.remove()
        a = self.axc.scatter(
            self.w, self.b, s=100, color=dlc["dlblue"], zorder=10, label="cost with \ncurrent w,b")
        b = self.axc.hlines(self.b, self.axc.get_xlim()[
                            0], self.w, lw=4, color=dlc["dlpurple"], ls='dotted')
        c = self.axc.vlines(self.w, self.axc.get_ylim()[
                            0], self.b, lw=4, color=dlc["dlpurple"], ls='dotted')
...
def plt_prob(ax, w_out, b_out):
    ...
    for i in range(tmp_x0.shape[0]):
        for j in range(tmp_x1.shape[1]):
            z[i, j] = sigmoid(np.dot(w_out, np.array(
                [tmp_x0[i, j], tmp_x1[i, j]])) + b_out)
