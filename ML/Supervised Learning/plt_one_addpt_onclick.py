...
from lab_utils_common import np, plt, dlblue, dlorange, dldarkred
from lab_utils_logistic import sigmoid, gradient_descent
...
class plt_one_addpt_onclick:
    ...
    def __init__(self, x, y, w, b, logistic=True):
        ...
        ax.scatter(x[pos], y[pos], marker='x',
        ...
        ax.text(0.65, 0.8, "[Click to add data points]",
        ...
        if self.logistic:
            self.bcalc = Button(
                axcalc, 'Run Logistic Regression (click)', color=dlblue)
            self.bcalc.on_clicked(self.calc_logistic)
        else:
            self.bcalc = Button(
                axcalc, 'Run Linear Regression (click)', color=dlblue)
    ...
    def add_data(self, event):
        ...
        if event.inaxes == self.ax[0]:
            ...
            if y_coord > 0.5:
                ...
                self.ax[0].scatter(x_coord, 0, marker='o', s=100,
                                   facecolors='none', edgecolors=dlblue, lw=3)
...
#   @output.capture()  # debug
    ...
    def draw_thresh(self):
        ...
        xlim = self.ax[0].get_xlim()
        a = self.ax[0].fill_between(
            [xlim[0], xp5], [ylim[1], ylim[1]], alpha=0.2, color=dlblue)
        b = self.ax[0].fill_between(
            [xp5, xlim[1]], [ylim[1], ylim[1]], alpha=0.2, color=dldarkred)
